...
class Demo(Fibermorph):
    ...
    def get_data(self, path, im_type):
        '''
            Download demo images from github, store them in the tmpdir
            input: Path to our input folder, image type
            output: The path to the input folder
        '''
        fiblog = self.get_logger('fiblog')
        #make the input directory
        datadir = pathlib.Path(path)
        tmpdir = self.make_directory(datadir, f"tmpdata_{im_type}", fiblog)
        demo_url = ''
        #download files
        if im_type == "curv":
            demo_url = [
        ...
        # Return input directory with data
        for u in demo_url:
    ...
    def run(self, args):
        """
            Downloads the data and runs the analysis
        ...
        # demo folder
        datadir = pathlib.Path(args.output_directory)
        fibermorph_demo_dir = self.make_directory(datadir, "fibermorph_demo", fiblog)

        # create input folder with data
        input_dir = self.get_data(fibermorph_demo_dir, demo_type)

        # output directory (within the demo folder)
        jetzt = datetime.now()
        timestamp = jetzt.strftime("%b%d_%H%M_")
